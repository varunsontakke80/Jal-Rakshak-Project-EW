...
void setup() {
  ...
  // Passing -1 to the SS argument prevents hardware lockup on the ESP32-S3 driver
...
void loop() {
  ...
  // DIAGNOSTIC CHECK: Did we catch any radio waves at all?
  if (packetSize) {
      Serial.println("\n====================================");
      Serial.printf(">>> RADIO WAVE DETECTED! <<<\n");
      Serial.printf("Packet Size Received : %d bytes\n", packetSize);
      Serial.printf("Struct Size Expected : %d bytes\n", sizeof(TelemetryPayload));
      Serial.printf("Signal Strength (RSSI): %d dBm\n", LoRa.packetRssi());
      
      // Check for struct size mismatch between ESP32 and ESP32-S3
      if (packetSize != sizeof(TelemetryPayload)) {
          Serial.println("[!] ERROR: Payload size mismatch. Packet dropped.");
          Serial.println("====================================\n");
          return; // Skip the rest of the loop
      }

      Serial.println("[SUCCESS] Perfect Payload Match! Unpacking data...");
      Serial.println("====================================\n");

      // Read the verified bytes
      ...
      // ONLY update the screen if the user is viewing the node that just transmitted
      if (current_listening_node > 0 && current_listening_node == received_node_id) {
          ...
          snprintf(displayBuf, sizeof(displayBuf),
                   ...
                   incomingData.waterPresence ? "YES" : "NO",
                   incomingData.leakAlert ? "YES" : "NO",
      ...
      if (firebase_initialized && firebase_app.ready() && received_node_id > 0 && received_node_id <= 4) {
          // Dynamically route data based on the embedded ID
