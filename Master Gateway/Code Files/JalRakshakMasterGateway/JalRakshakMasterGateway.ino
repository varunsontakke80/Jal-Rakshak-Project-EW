...
#include <lvgl.h>
#include <time.h> // Required for NTP Date/Time syncing
...
// --- NTP Time Configuration (IST = UTC + 5:30) ---
...
// ------------------------------------------------------------------
// FINAL C-STRUCT PAYLOAD (Matched to Pipe Node)
// ------------------------------------------------------------------
#pragma pack(push, 1)
...
// ==========================================
// UTILITY: GET FORMATTED DATE/TIME
// ==========================================
...
// ==========================================
// UTILITY: LOG CUP TEST TO FIREBASE
// ==========================================
void logTestToFirebase(float nh3, float h2s, String result) {
    if (firebase_initialized && firebase_app.ready()) {
        // Create a unique database push ID based on millis() to prevent overwriting
        String testPath = "/lab_results/" + String(millis()); 
        
        Database.set<float>(aClient, testPath + "/ammonia", nh3, processData, "log_nh3");
        Database.set<float>(aClient, testPath + "/h2s", h2s, processData, "log_h2s");
        Database.set<String>(aClient, testPath + "/result", result, processData, "log_res");
        
        // Use the NTP synchronized time for Streamlit Table logging
        Database.set<String>(aClient, testPath + "/timestamp", getFormattedTime(), processData, "log_ts");
    }
}
...
void setup() {
  ...
  // 2. Initialize Internal ADC & MQ Math Models
  analogReadResolution(12); // Lock resolution to 12-bit (0-4095)
  ...
  // 3. Init LoRa with DEDICATED SPI BUS (Passing -1 prevents SPI driver lockup)
  ...
  if (!LoRa.begin(433E6)) {
    ...
    Serial.println("LoRa Driver Online. Clear Channel Set.");
  }
...
void loop() {
  ...
  if (packetSize == sizeof(TelemetryPayload)) {
      ...
      int received_node_id = incomingData.nodeId;
      ...
      // Only update the display if the user is looking at the node that just broadcasted
      if (current_listening_node > 0 && current_listening_node == received_node_id) {
          char displayBuf[512];
      ...
      if (firebase_initialized && firebase_app.ready() && received_node_id > 0 && received_node_id <= 4) {
          String basePath = "/nodes/node_" + String(received_node_id);
          
          Database.set<float>(aClient, basePath + "/latitude", incomingData.latitude, processData, "lat");
          Database.set<float>(aClient, basePath + "/longitude", incomingData.longitude, processData, "lon");
          ...
          // Send a timestamp marker
          Database.set<String>(aClient, basePath + "/last_updated", getFormattedTime(), processData, "ts");
      }
  }
}
  ...
  lv_obj_add_event_cb(btn_connect, [](lv_event_t * e){
      ...
      wifi_timer = lv_timer_create([](lv_timer_t * t){
        if(WiFi.status() == WL_CONNECTED) {
          ...
          // FIREBASE INITIALIZATION & TIME SYNC TRIGGER
          if (!firebase_initialized) {
              ssl_client.setInsecure();
              ssl_client.setConnectionTimeout(1000);
              ssl_client.setHandshakeTimeout(5);
              
              // Sync Time over NTP for Firebase Timestamps
...
// ==========================================
// 8. WATER TEST SCREEN (MQUnifiedsensor)
// ==========================================
void build_scr_water() {
  ...
  auto reset_water_screen = [](lv_event_t * e){
    if(water_timer != NULL) { lv_timer_del(water_timer);
      water_timer = NULL; }
    lv_bar_set_value(water_bar, 0, LV_ANIM_OFF);
    lv_obj_add_flag(water_bar, LV_OBJ_FLAG_HIDDEN);
  ...
  lv_obj_add_event_cb(btn_test_now, [](lv_event_t * e){
    ...
    water_timer = lv_timer_create([](lv_timer_t * t){
      ...
      if(water_progress >= 100) {
        ...
        // --- AUTOMATED LAB FIREBASE LOGGING ---
        logTestToFirebase(nh3_ppm, h2s_ppm, status);
      }
    }, 200, NULL);
  }, LV_EVENT_CLICKED, NULL);
}